...
const WORD = 'block text-[clamp(3rem,15vw,7rem)] font-semibold leading-[0.85] tracking-tighter'

const WORDS: { text: string; align: string }[] = [
  { text: 'track', align: 'self-start' },
  { text: 'every', align: 'self-center' },
  { text: 'swing', align: 'self-end' },
]
...
export function SwingHeadline() {
  ...
  if (reduced) {
    return (
      <h1 className="mt-6 flex flex-col text-white">
        {WORDS.map((w) => (
          <span key={w.text} className={cn(WORD, w.align)}>
            {w.text}
          </span>
        ))}
      </h1>
  ...
  return (
    <div className="relative mt-6">
      ...
      <h1 className="relative z-[1] flex flex-col text-white" aria-label="track every swing">
        {WORDS.map((w, wi) => (
          <span key={w.text} className={cn(WORD, w.align)} aria-hidden="true">
            {w.text.split('').map((ch) => {
              const i = letterIndex++
